...
def cl_forward(cls,
    ...
    if disable_dropout and momentum > 0. :
        ...
        if momentum > 0.:
            for param, param_m in zip(encoder.parameters(), cls.momentum_model.parameters()):
...
class BertForCL(BertPreTrainedModel):
    ...
    def __init__(self, config, *model_args, **model_kargs):
        ...
        if self.model_args.momentum > 0.:
            self.momentum_model = BertModel(config)
            for param, param_m in zip(self.bert.parameters(), self.momentum_model.parameters()):
...
class RobertaForCL(RobertaPreTrainedModel):
    ...
    def __init__(self, config, *model_args, **model_kargs):
        ...
        if self.model_args.momentum > 0.:
            self.momentum_model = RobertaModel(config)
            for param, param_m in zip(self.roberta.parameters(), self.momentum_model.parameters()):
                param_m.data.copy_(param.data)  # initialize
                param_m.requires_grad = False  # not update by gradient  
        if self.model_args.do_mlm:
